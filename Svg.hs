...
svgCircle (Circle p@(x :+ y) r) color =
   do
     ...
     text_ [x_ $ ps $ size * x, y_ $ ps $ size * y, font_size_ $ fromString "7"] $ fromString $ roundComplex p
...
svgCirclesColor [] _ = return ()
svgCirclesColor (c:cs) color = do
  svgCircle c (fromString color)
  svgCirclesColor cs color
...
svgBoxes ((Box (x0 :+ y0) (x1 :+ y1)):bs) =
  let dx = x1 - x0
      dy = y1 - y0
  in do rect_ [width_ $ ps $ size * dx, height_ $ ps $ size * dy, x_ $ ps $ size * x0, y_ $ ps $ size * y0, fill_ $ fromString "red"]
        svgBoxes bs
